"""CE Compliance Report — PDF generator for any state licensing board.

Replaces tmb_report.py. The PDF header dynamically shows the correct board name
based on the user's primary license state.
"""
from datetime import datetime
from collections import defaultdict
from sqlalchemy.orm import Session
from models import User, License, CEU, Credential, StateRequirement
from board_names import get_board_name


def get_primary_board_name(db: Session, user_id: int) -> str:
    """Resolve board name from the user's primary (first) license state."""
    primary = (
        db.query(License)
    ...
    req = (
        db.query(StateRequirement)
        .filter(StateRequirement.state == primary.state)
        .first()
    )
    if req and req.board_name:
        return req.board_name
    return get_board_name(primary.state)


def generate_ce_report_html(user: User, licenses: list, ceus: list, credentials: list,
                            board_name: str = "State Licensing Board") -> str:
    """Generate HTML for the CE Compliance Report PDF."""
...
<body>
    ...
    <div class="report-header">
        <div class="agency">{board_name}</div>
    ...
    <p style="font-size:9pt;color:#666;font-style:italic;">
        Note: This report documents {total_credits:.1f} CEUs earned during the current {cycle_years}-year renewal cycle.
        Check with {board_name} for specific submission requirements (audit, online portal, or CE Broker).
    </p>
...
def generate_ce_pdf(user: User, licenses: list, ceus: list, credentials: list,
                    board_name: str = "State Licensing Board") -> bytes:
    """Generate CE compliance PDF using WeasyPrint. Returns PDF bytes."""
    from weasyprint import HTML
    html_content = generate_ce_report_html(user, licenses, ceus, credentials, board_name)
    pdf = HTML(string=html_content).write_pdf()
    return pdf


def generate_ce_report(db: Session, user_id: int) -> bytes:
    """Fetch data from DB and generate full CE compliance report PDF."""
    ...
    board_name = get_primary_board_name(db, user_id)
    return generate_ce_pdf(user, licenses, ceus, credentials, board_name)


# Backward-compatible shim for any code still importing from tmb_report
def generate_tmb_report(db: Session, user_id: int) -> bytes:
    """Legacy alias — delegates to generate_ce_report."""
    return generate_ce_report(db, user_id)